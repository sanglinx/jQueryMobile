...
$.widget( "mobile.pagecontainer", {
	...
	_filterNavigateEvents: function( e, data ) {
        // Add replaceAll ProtoType for String
        String.prototype.replaceAll = function(search, replacement) {
            var target = this;
            return target.replace(new RegExp(search, 'g'), replacement);
        };

        var currentUrl = new URL(data.state.url);
        var tempParam = currentUrl.search;
        // Replace All HTTP[S] hostname with emppty value
        currentUrl.search = tempParam.replaceAll(/(http[s]?:\/\/.*?(?=\/))/gm,"");
        data.state.url = currentUrl.toString();
        data.state.hash = data.state.hash.replace(tempParam, currentUrl.search);
